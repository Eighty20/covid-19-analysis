---
title: "Data Retrieval for Covid Modelling"
author: "Murray"
date: "5/8/2020"
output: html_document
---

Retrieves the following data from various sources:

  1)  Age distribution by country
  2)  Number of coronavirus tests, cases, and fatalities by country (needs to be re-run when required to keep up to date)
  3)  Obesity rates by country and gender
  4)  Overweight rates by country and gender
  5)  Government response data (needs to be re-run when required to keep up to date)
  6)  GDP (PPP) for each country
  7)  Infection / Death proportions by age group (from Panda Model) (should be updated to take in latest data)
...
# 1 - Age Distributions by Country
...
read.csv(url(paste0("https://www.populationpyramid.net/api/pp/", i, "/2019/?csv=true")))
...
# Takes ~ 15 minutes
# Cote d'Ivoire is number 384, Curacao is 531, but both have unrecognised characters in name so just leaving them out for now
# Monaco is number 492, contains no data so excluded.
...
Get median age for each country - https://www.cia.gov/library/publications/resources/the-world-factbook/fields/343rank.html
...
# Unfortunately, the above data doesn't read in well, so we need to do some cleaning:
...
tests_cases_deaths_data[tests_cases_deaths_data$location == "United Kingdom",]
tests_cases_deaths_data[tests_cases_deaths_data$location == "Spain",]
tests_cases_deaths_data[tests_cases_deaths_data$location == "Italy",]


sum(!is.na(tests_cases_deaths_data$total_tests))
sum(!is.na(tests_cases_deaths_data$total_cases))
sum(!is.na(tests_cases_deaths_data$total_deaths))

(tests_cases_deaths_data[!is.na(tests_cases_deaths_data$total_tests),])[3000:4000,]
...
# Only keep the point estimates. Some NAs de to missing data
...
gov_response_data %>% filter(CountryName == "Brazil") %>% select(StringencyIndexForDisplay)

mean(gov_response_data$StringencyIndexForDisplay, na.rm = T)
```
...
unique(gdp_per_capita_ppp_data$Subject.Descriptor)
unique(gdp_per_capita_ppp_data$Units)
unique(gdp_per_capita_ppp_data$Scale)
unique(gdp_per_capita_ppp_data$Country.Series.specific.Notes)
...
# Proportion of deaths by age group. We use Sweden and Italy's statistics.
...
population_fatality_rates_relative_to_Italy <- cbind.data.frame(names(age_data),
                                              population_fatality_rate / population_fatality_rate[names(age_data) == "Italy"])

names(population_fatality_rates_relative_to_Italy) <- c("Country", "fatality_rate")

population_fatality_rates_relative_to_Italy[order(population_fatality_rates_relative_to_Italy[,2]),]
```

```{r}
median_age_data %>% filter(Country == "Russia")
