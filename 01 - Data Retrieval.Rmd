...
Only the age data takes time to load, so just read in the Rds for that. Rest should be quick. 
...
Retrieves the following data from various sources:
  ...
  7)  Infection / Death proportions by age group (doesn't need to be updated often; just used for relative risks, not absolute)
  8)  Population density data
...
# Takes ~ 15 minutes and throws a lot of scary red text - rather just read in the Rds at the bottom of the chunk
...
saveRDS(age_data, file = "age_data.Rds")

age_data <- readRDS(file = "age_data.Rds")
```

Now Get median age for each country - https://www.cia.gov/library/publications/resources/the-world-factbook/fields/343rank.html
...
gov_response_data %>% filter(CountryName == "Sweden")
...
# Calculate risk-adjusted age of countries using deaths per 100 000 by age group and population pyramids. We use Sweden, Netherlands and Italy's statistics.
...
names(risk_adjusted_age) <- c("Country", "risk_adjusted_age")

risk_adjusted_age$Country <- as.character(risk_adjusted_age$Country)

risk_adjusted_age[order(risk_adjusted_age[,2]),]
```
...
# Read in Vincent's density parameters


```{r}
density_data <- read.csv("country_density_dat.csv")
```
