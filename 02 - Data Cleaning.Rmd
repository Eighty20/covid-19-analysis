...
# simple_model_data <-
#   simple_model_data %>%
#     left_join(tests_cases_deaths_data %>%
...
simple_model_data %>% filter(Country == "MicronesiaFedStatesof" | Country == "Micronesia")
...
simple_model_data <-
  simple_model_data %>%
    left_join(density_data %>%
                select(Country, Density_km, Inequality_spatial_transformed))

```

# Join government response data
...
simple_model_data <- 
  simple_model_data %>%
    left_join(gov_response_data %>%
                select(-CountryCode, -ConfirmedCases, -ConfirmedDeaths, -StringencyIndex, -LegacyStringencyIndex) %>%
                rename(StringencyIndex = StringencyIndexForDisplay,
                       LegacyStringencyIndex = LegacyStringencyIndexForDisplay))

```
# Checking which is better of StringencyIndex and StringencyIndexForDisplay

```{r}
# sum(gov_response_data$StringencyIndex - gov_response_data$StringencyIndexForDisplay > 0, na.rm = T)
# 
# sum(gov_response_data$LegacyStringencyIndex - gov_response_data$LegacyStringencyIndexForDisplay > 0, na.rm = T)
# 
# sum(is.na(gov_response_data$StringencyIndex))
# sum(is.na(gov_response_data$StringencyIndexForDisplay))
# sum(is.na(gov_response_data$StringencyIndexForDisplay) & !is.na(gov_response_data$StringencyIndex))
# 
# sum(is.na(gov_response_data$LegacyStringencyIndex))
# sum(is.na(gov_response_data$LegacyStringencyIndexForDisplay))
# sum(is.na(gov_response_data$LegacyStringencyIndexForDisplay) & !is.na(gov_response_data$LegacyStringencyIndex))
```
