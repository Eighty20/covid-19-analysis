...
# Enter in Diamond Princess (dp) data
...
```{r}
full_data <- rbind.data.frame(rep(0, 145))[-1,]
...
for(i in intersect(covid_data$location, names(age_data))) {
  ...
  curr_data <-
    covid_data %>%
      ...
      select(location, current_total_deaths, expected_final_deaths, population, expected_final_deaths_per_person, date, new_deaths) %>%
      ungroup() %>%
      pivot_wider(names_from = date, values_from = new_deaths) %>%
      filter(location == i) %>%
      mutate(expected_final_deaths_per_person = expected_final_deaths / population)
